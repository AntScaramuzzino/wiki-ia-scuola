---
title: "Formare all'intelligenza artificiale: un progetto-studio con docenti e futuri docenti"
type: source
tags: [ai, educazione, apprendimento, pedagogia, competenze, alfabetizzazione, ai-formazione]
created: 2026-06-21
updated: 2026-06-21
sources: ["PAPER | Formare all'intelligenza artificiale: un progetto-studio con do- centi e futuri docenti - FrancoAngeli Journals"]
---

**File raw**: `raw/papers/formare-all-intelligenza-artificiale-un-progetto-studio-con-do-centi-e.md`
**Origine**: NotebookLM «Linee Guida sull'Intelligenza Artificiale a Scuola»

## Sommario
Paper accademico di Roberta Scarano e Concetta Ferrantino (Università degli Studi di Salerno), pubblicato su *Education Sciences & Society* 2/2024 (FrancoAngeli). Argomenta l'urgenza della formazione iniziale e continua dei docenti sull'IA e presenta il disegno di uno studio sperimentale (pre-sperimentale con post-test e gruppi non equivalenti) condotto con docenti in formazione e futuri docenti di Scienze della Formazione Primaria a Salerno. L'ipotesi è che senza una formazione adeguata l'impiego dell'IA a scuola non possa essere efficace né integrato nella progettazione curricolare.

## Punti chiave
- La competenza digitale è requisito di cittadinanza attiva; l'Italia è in coda nell'indice DESI 2023 (45,75% con competenze digitali di base, 22,21% sopra il livello base).
- Distingue **AI Literacy** (insegnare *di* IA: cos'è, come funziona, implicazioni etiche) da **AIED / AIEd** (Artificial Intelligence in Education: usare l'IA come supporto pedagogico-didattico per personalizzare l'apprendimento).
- Adotta il framework di **AI Literacy** di Cuomo, Biagini e Ranieri (2022) con quattro dimensioni: conoscitiva, operativa, critica ed etica.
- Il docente resta un mediatore insostituibile (*exemplum* etico): l'IA può solo mediare l'esperienza di insegnamento-apprendimento, non sostituire il ruolo docente.
- Per la formazione di adulti propone uno spostamento dal modello pedagogico a quello **andragogico** e l'uso del **self-directed learning** e della formazione attiva.
- Disegno dello studio: 30 ore di formazione (5h frontali + 20h attive con coding, Scratch, chatbot, visori + 5h di feedback con Kahoot!, brainstorming, debate) per il gruppo sperimentale vs spiegazione tradizionale per il gruppo di controllo.
- Critica i corsi esistenti centrati su aspetti tecnici ed etici dell'IA che trascurano la dimensione pedagogico-didattica (Ottone, 2024).
- Quadro normativo nazionale citato: PNSD (azioni 25 e 27), Legge 107/2015, Missione 4 PNRR, Piano Scuola 4.0, DigCompEdu, Legge 92/2019 (cittadinanza digitale).

## Entità menzionate
[[entity/ocse|OCSE (OECD)]] — ha sottolineato l'importanza della consapevolezza dei docenti nell'uso delle ICT (richiamata dal PNSD azione 27)
[[entity/mim|Ministero dell'Istruzione e del Merito (MIM)]] — riferimenti a MIUR/Indicazioni Nazionali e Nuovi scenari (2018), PNSD, Piano Scuola 4.0

## Concetti trattati
[[concept/alfabetizzazione-ia|Alfabetizzazione all'IA (AI Literacy)]] — concetto centrale (AI Literacy e suo framework a quattro dimensioni)
[[concept/intelligenza-artificiale|Intelligenza Artificiale]] — oggetto formativo
[[concept/machine-learning|Machine Learning]] — incluso nella dimensione conoscitiva dell'AI Literacy
[[concept/ia-generativa|IA Generativa]] — citata in relazione a scrittura, plagio, ChatGPT
[[concept/didattica-inclusiva-ia|Didattica inclusiva con IA]] — l'IA come supporto a inclusione, personalizzazione e riduzione di stereotipi di genere in STEM
[[concept/gamification|Gamification]] — uso di Kahoot! e attività ludiche nella formazione attiva

## Citazioni rilevanti
> "L'uso di sistemi di IA generativa nell'insegnamento-apprendimento richiede la conoscenza e la comprensione di tali sistemi sia da parte del docente e del formatore, sia da parte dell'allievo." (Ottone, 2024)

> "L'IA può solo mediare l'esperienza di insegnamento-apprendimento, ma […] restano loro [docenti e studenti] i costruttori attivi del processo apprenditivo."

## Contraddizioni o tensioni
Nessuna contraddizione rilevata. La fonte fornisce la cornice pedagogico-scientifica (framework AI Literacy a 4 dimensioni, distinzione AI Literacy vs AIED, modello andragogico) che integra e arricchisce le fonti normative del wiki ([[source/linee-guida-mim-dm166-2025|Linee Guida MIM per l'IA a scuola (DM 166/2025)]], [[source/oecd-ai-literacy-2025|OECD AI Literacy Framework (2025)]]).

## Domande aperte
- Risultati empirici dello studio: il paper presenta solo il disegno, non i dati raccolti.
- Il framework di Cuomo/Biagini/Ranieri meriterebbe una pagina concept dedicata (vedi suggerimenti).
