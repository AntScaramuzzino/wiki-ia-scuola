---
title: "L'intelligenza artificiale entra nelle scuole italiane: ecco le linee guida del Ministero (FPA)"
type: source
tags: [ai, educazione, normativa, politiche, privacy, governance-scolastica-ia, ai-formazione, pnrr]
created: 2026-06-21
updated: 2026-06-21
sources: ["L'intelligenza artificiale entra nelle scuole italiane: ecco le linee guida del Ministero"]
---

**File raw**: `raw/papers/l-intelligenza-artificiale-entra-nelle-scuole-italiane-ecco-le-linee-g.md`
**Origine**: NotebookLM «Linee Guida sull'Intelligenza Artificiale a Scuola»

## Sommario
Articolo di approfondimento di FORUM PA (Claudia Bertozzi, 12 settembre 2025) che illustra in modo strutturato e dettagliato i contenuti delle Linee Guida MIM allegate al DM 166/2025. Pur essendo un commento giornalistico, fornisce un'analisi articolata e ricca di dettagli operativi (ruolo di "deployer", FRIA, divieti, obblighi per i sistemi ad alto rischio) che vanno oltre la semplice notizia di lancio.

## Punti chiave
- Le Linee Guida sono parte integrante del **DM n. 166 del 09/08/2025** e si attuano tramite un nuovo servizio digitale nella **piattaforma Unica**.
- **Approccio antropocentrico**: l'IA è strumento al servizio della crescita umana; ogni decisione importante sugli studenti richiede supervisione umana qualificata.
- Quattro elementi del modello MIM: principi di riferimento (centralità della persona, equità, innovazione etica, sostenibilità, sicurezza), requisiti di base etico-tecnico-normativi, framework di implementazione in cinque fasi, sistema di comunicazione e governance.
- La scuola passa da semplice utente a **"deployer"** (definizione AI Act): soggetto giuridico e tecnico responsabile, con obblighi di gestione del rischio.
- Approccio basato sul rischio: rischio nullo/minimo, limitato (trasparenza), alto (obblighi stringenti).
- **Divieti**: tecniche subliminali/manipolative, social scoring e — elemento distintivo per la scuola — riconoscimento delle emozioni degli studenti (salvo eccezioni di sicurezza/medico).
- Sistemi ad alto rischio in ambito educativo: ammissione, valutazione dell'apprendimento, assegnazione a percorsi, monitoraggio anti-cheating durante gli esami, profilazione.
- Tre obblighi per la scuola-deployer: uso conforme, supervisione umana qualificata, valutazione d'impatto integrata **DPA + FRIA** (con consultazione del Garante in caso di rischi residui).
- Connessioni con Strategia nazionale IA 2024-2026 (AgID), PNSD, fondi PNRR (sperimentazione biennale in 15 classi in 4 regioni, campus innovativi).
- Applicazioni concrete per dirigenti, personale amministrativo, docenti (integrazione con PEI e PDP) e studenti.

## Entità menzionate
[[entity/mim|Ministero dell'Istruzione e del Merito (MIM)]] — autore delle Linee Guida e del DM 166/2025
[[entity/agid|Agenzia per l'Italia Digitale (AgID)]] — Strategia nazionale per l'IA 2024-2026
[[entity/garante-privacy|Garante per la protezione dei dati personali]] — da consultare in caso di rischi residui elevati
[[entity/commissione-europea|Commissione Europea (e JRC)]] — AI Act e approccio basato sul rischio

## Concetti trattati
[[concept/governance-scolastica-ia|Governance scolastica dell'IA]] — modello a quattro elementi, ruolo di deployer
[[concept/supervisione-umana|Supervisione umana (human-in-the-loop)]] — requisito cardine per i sistemi ad alto rischio
[[concept/privacy-protezione-dati|Privacy e protezione dei dati]] — DPA, GDPR, privacy by design/default, dati sintetici
[[concept/bias-algoritmico|Bias Algoritmico]] — discriminazione algoritmica e principio di equità
[[concept/allucinazione|Allucinazione]] — da far riconoscere agli studenti
[[concept/didattica-inclusiva-ia|Didattica inclusiva con IA]] — integrazione con PEI e PDP
[[concept/ia-generativa|IA Generativa]] — già entrata in aula, "rivoluzione silenziosa"
[[concept/snodi-formativi-pnrr|Snodi formativi PNRR (DM 219/2025)]] — fondi PNRR per sperimentazioni e campus

## Citazioni rilevanti
> "La tecnologia deve supportare il pieno sviluppo della persona, garantendo il ruolo centrale e insostituibile dell'uomo nel governo dei sistemi di IA"

> obiettivo dichiarato: costruire "una scuola più inclusiva, equa e preparata ad affrontare le sfide del futuro"

## Contraddizioni o tensioni
Commento/approfondimento di [[source/linee-guida-mim-dm166-2025|Linee Guida MIM per l'IA a scuola (DM 166/2025)]]; non introduce fatti normativi nuovi rispetto alla fonte canonica, ma ne fornisce una rilettura analitica dettagliata (concetto di deployer, FRIA, divieti) utile come sintesi divulgativa. Coerente con [[source/ai-act-ue|AI Act — Regolamento (UE) 2024/1689]] sull'approccio basato sul rischio.

## Domande aperte
- Come verranno standardizzati i criteri di scelta dei fornitori e l'effettiva attuazione della supervisione umana nelle singole scuole?
- Quali decreti attuativi chiariranno le finalità specifiche del trattamento dei dati personali?
